...
const smoke = path.join(process.cwd(), 'scripts', 'follower', 'renderer_start_flow_smoke.cjs');
const env = { ...process.env };
delete env.ELECTRON_RUN_AS_NODE;
const virtual = !['win32', 'darwin'].includes(process.platform);
const command = virtual ? 'xvfb-run' : electronPath;
const args = virtual ? ['-a', electronPath, '--no-sandbox', '--disable-gpu', smoke] : ['--disable-gpu', smoke];
